...
/**
 * Proyek Kelompok: The End Of The Day
 */
...
public class TheEndOfTheDay {
    ...
    static void gameloop() {
        ...
        while (gameRunning) {
            ...
            if (player.GetInfected()) {
                System.out.println("> PERINGATAN: Virus menyebar di tubuhmu! HP berkurang.");
    ...
    static void actionMenu() {
        ...
        System.out.println("6. Istirahat");
        System.out.println("7. Cek Status Misi");
        ...
        switch (pilihan) {
            ...
            case 5: hide(); break;
            case 6: rest(); break;
            case 7: mission.showMissionStatus(); break;
    ...
    static void makan() {
        ...
        System.out.println("> Lapar berkurang. Sisa makanan: " + foodStock + " kaleng.");
    }
    ...
    static void pindahLokasi() {
        ...
        if (rand.nextInt(3) == 0) {
            System.out.println("> Di perjalanan, musuh menghadangmu!");
            delay();
            battle();
    ...
    static void jelajahiArea() {
        ...
        int peluangMusuh = 10 + (turn / 5) * 5; 
        if (rand.nextInt(100) < peluangMusuh) {
            System.out.println("> Kamu mendengar suara langkah berat...");
            delay();
            System.out.println("> Makhluk itu muncul dari kegelapan!");
            battle();
            if (player.getHP() <= 0) return;
        }
        ...
        if (loc.hasClueStored() && !loc.isClueFound()) {
            if (rand.nextInt(2) == 0) { 
                ...
                System.out.println("> " + loc.getClueStoredDesc());
                System.out.println("> Ambil? (1=Ya / 2=Tidak): "); // Ubah print menjadi println
        ...
        if (!loc.isItemComplete()) {
            ...
            if (rand.nextInt(100) < 40) {
                System.out.println("> Ada yang menjaga item ini!");
                battle();
                if (player.getHP() <= 0) return;
            }
            ...
            if (berhasil) {
                ...
                if (loc.isItemComplete()) {
                    ...
                    System.out.println("\n>>> GEMURUH DARI DALAM " + loc.getName().toUpperCase() + "! <<<");
                    delay();
    ...
    static void findweapon () {
        ...
        System.out.println("Damage kamu meningkat menjadi: " + player.getDamage());
    }
    ...
    static void battle() {
        Zombie musuh = spawnZombie(); 
        System.out.println("> Makhluk muncul: " + musuh.getType());

        while (musuh.HP > 0 && player.getHP() > 0) {
            System.out.println("\n1. Serang");
            ...
            if (c == 1) { 
                musuh.HP -= player.getDamage();
                System.out.println("> Kamu menyerang! Damage: " + player.getDamage());

                if (musuh.HP <= 0) {
                    // PERBAIKAN: BUG DYNAMITE LOGIC
                    if (musuh instanceof Dynamite) {
                        System.out.println(">>> !! PERINGATAN: DYNAMITE ZOMBIE MELEDAK !! <<<");
                        int damageLedakan = 35;
                        player.setHP(Math.max(0, player.getHP() - damageLedakan));
                        System.out.println("> DUARRR! Kamu terkena hempasan ledakan dan menerima " + damageLedakan + " damage!");
                    }
                    
                    System.out.println("> Musuh mati!");
           
                    if (rand.nextInt(6) == 0) {
                        System.out.println("> Makhluk itu menjatuhkan kaleng makanan! +1 makanan.");
                        foodStock++;
                    }
                } else {
                    musuh.attack(player);
                ...
                if (rand.nextBoolean()) { 
                    ...
                    System.out.println("> Gagal kabur! Musuh menyerangmu!");
                    musuh.attack(player);
                }
            }
        }
    ...
    static void randomEvent() {
        ...
        if (r == 1) {
            ...
            if (c == 1) {
                ...
                player.setHP(Math.min(200, player.getHP() + 20)); // PERBAIKAN: MAX LIMIT HP
            } else {
        ...
        if (r == 2) {
            ...
            if (c == 1) {
                ...
                player.setHP(Math.min(200, player.getHP() + 15)); // PERBAIKAN: MAX LIMIT HP
                if (player.GetInfected()) { 
                    player.setInfected(false); 
                    System.out.println("> Infeksi mereda!"); 
                }
            } else {
        ...
        if (r == 4) {
            ...
            delay();
            System.out.println("> Musuh melambat. Kamu bisa beristirahat sejenak. HP +5, Stamina +10");
            player.setHP(Math.min(200, player.getHP() + 5)); // PERBAIKAN: MAX LIMIT HP
            player.setStamina(Math.min(100, player.getStamina() + 10)); // PERBAIKAN: MAX LIMIT STAMINA
        }
        ...
        if (r == 6) {
            System.out.println("\n> Musuh datang tiba-tiba dari gang sempit!");
            delay();
            battle();
        }
    }
    ...
    static void hungerSystem() {
        player.setHunger(Math.min(100, player.getHunger() + 5)); // PERBAIKAN: MAX LIMIT LAPAR
        if (player.getHunger() >= 50) { 
            System.out.println("> Kamu kelaparan! HP -5"); 
            player.setHP(Math.max(0, player.getHP() - 5)); 
        }
    }
    ...
    static void hide (){
        System.out.println("Kamu memilih untuk bersembunyi dari para makhluk tersebut...");
        delay();
        if (rand.nextBoolean()){
            System.out.println("Kamu berhasil bersembunyi dengan aman.");
        }
        else{
            System.out.println("Kamu gagal bersembunyi!! Ada yang menemukanmu!!");
        }
    }
    ...
    static void rest(){
        ...
        player.setStamina(Math.min(100, player.getStamina() + 10)); // PERBAIKAN: MAX LIMIT STAMINA
        player.setHP(Math.min(200, player.getHP() + 10)); // PERBAIKAN: MAX LIMIT HP
    }
